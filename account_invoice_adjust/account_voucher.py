...
class account_voucher(osv.osv):
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        t = super(account_voucher,self).action_move_line_create(cr,uid,ids,context)
        for voucher in self.browse(cr, uid, ids, context=context):
            if voucher.move_id:
                for i in voucher.move_id.line_id:
                    if i.credit == 0 and i.debit==0:
                        self.pool.get('account.move.line').unlink(cr,uid,[i.id],context)
                        print "llegue"
        return t
