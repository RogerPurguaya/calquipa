...
class costos_produccion(models.Model):
	...
	def crear_trituracion(self):
		...
		and sm.state = 'done' and pt.type='product'
		and fecha_num(sp.date::date) >= fecha_num('""" + str(self.periodo.date_start) + """'::date) and fecha_num(sp.date::date) <= fecha_num('""" + str(self.periodo.date_stop) + """'::date)
		and sp.motivo_guia = '17'
		order by sp.date
		 """)

		for i in self.env.cr.fetchall():
			m = self.env['stock.move'].search([('id','=',i[0])])[0]
			m.precio_unitario_manual = self.c_uni_tri*2
			m.price_unit = self.c_uni_tri*2
	...
	def crear_micronizado(self):
		...
		and sm.state = 'done' and pt.type='product'
		and coalesce(sp.motivo_guia,'0') != '10'
		and fecha_num(sp.date::date) >= fecha_num('""" + str(self.periodo.date_start) + """'::date) and fecha_num(sp.date::date) <= fecha_num('""" + str(self.periodo.date_stop) + """'::date)
		order by sp.date
		 """)
	...
	def get_tritu_final_ton(self):
		...
		self.tritu_final_ton = self.tritu_dis_ton - self.tritu_tt_ton - self.tritu_ven_ton - self.tritu_repro_ton_d
	...
	def get_tritu_final_imp(self):
		...
		self.tritu_final_imp = self.tritu_dis_imp - self.tritu_tt_imp - self.tritu_ven_imp - self.tritu_repro_imp_d
	...
	def get_tritu_repro_cp(self):
		if self.tritu_repro_ton == 0:
			self.tritu_repro_cp = 0
		else:
			self.tritu_repro_cp = self.tritu_repro_imp / self.tritu_repro_ton
	



	@api.one
	def get_tritu_repro_ton_d(self):	
		self.tritu_repro_ton_d = self.tritu_repro_ton
		

	@api.one
	def get_tritu_repro_imp_d(self):	
		self.tritu_repro_imp_d = self.tritu_repro_imp / 2
		

	@api.one
	def get_tritu_repro_cp_d(self):
		if self.tritu_repro_ton_d == 0:
			self.tritu_repro_cp_d = 0
		else:
			self.tritu_repro_cp_d = self.tritu_repro_imp_d / self.tritu_repro_ton_d
	...
	def get_tritu_dis_ton(self):
		self.tritu_dis_ton = self.tritu_ini_ton + self.tritu_pro_ton + self.tritu_repro_ton

	@api.one
	def get_tritu_dis_imp(self):
		self.tritu_dis_imp = self.tritu_ini_imp + self.tritu_pro_imp + self.tritu_repro_imp
	...
	tritu_repro_ton = fields.Float('Reproceso',compute="get_tritu_repro_ton",digits=(12,2))
	tritu_repro_imp = fields.Float('Reproceso',compute="get_tritu_repro_imp",digits=(12,2))
	tritu_repro_cp = fields.Float('Reproceso',compute="get_tritu_repro_cp",digits=(12,2))



	tritu_repro_ton_d = fields.Float('Reproceso',compute="get_tritu_repro_ton_d",digits=(12,2))
	tritu_repro_imp_d = fields.Float('Reproceso',compute="get_tritu_repro_imp_d",digits=(12,2))
	tritu_repro_cp_d = fields.Float('Reproceso',compute="get_tritu_repro_cp_d",digits=(12,2))
	...
	@api.one
	def get_calci_salida_cp(self):
		if self.calci_salida_ton == 0:
			self.calci_salida_cp = 0
		else:
			self.calci_salida_cp = self.calci_salida_imp / self.calci_salida_ton
	...
	calci_salida_ton = fields.Float('Otros',compute="get_calci_salida_ton")
	calci_salida_imp = fields.Float('II Importe',compute="get_calci_salida_imp")
	calci_salida_cp = fields.Float('II Costo Prom.',compute="get_calci_ven_cp",digits=(12,6))
	...
	@api.one
	def get_micro_merc_cp(self):
		if self.micro_merc_ton == 0:
			self.micro_merc_cp = 0
		else:
			self.micro_merc_cp = self.micro_merc_imp / self.micro_merc_ton
	...
	micro_merc_ton = fields.Float('Mercaderia',compute="get_micro_merc_ton",digits=(12,2))
	micro_merc_imp = fields.Float('II Importe',compute="get_micro_merc_imp",digits=(12,2))
	micro_merc_cp = fields.Float('II Costo Prom.',compute="get_micro_merc_cp",digits=(12,6))
