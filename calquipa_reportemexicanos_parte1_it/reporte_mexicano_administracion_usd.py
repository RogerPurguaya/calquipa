...
class rm_report_administracion_line(models.Model):
	_inherit = 'rm.report.administracion.line'

	@api.one
	def get_acumulado_usd(self):
		periodos = self.env['account.period'].search([('fiscalyear_id','=',self.rm_report_administracion_id.fiscal.id)])
	...
	def get_acumulado_pciento_usd(self):
		if self.acumulado_usd != 0:
			self.acumulado_pciento_usd = self.acumulado_usd / self.rm_report_administracion_id.total_general_usd
		else:
	...
	def get_promedio_pciento_usd(self):
		if self.acumulado_usd != 0:
			self.promedio_pciento_usd = self.promedio_usd / self.rm_report_administracion_id.total_promedio_general_usd
		else:
