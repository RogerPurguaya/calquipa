...
class rm_report_intercompanias_line(models.Model):
	_inherit = 'rm.report.intercompanias.line'

	@api.one
	def get_acumulado_usd(self):
		periodos = self.env['account.period'].search([('fiscalyear_id','=',self.rm_report_intercompanias_id.fiscal.id)])
	...
	def get_acumulado_pciento_usd(self):
		if self.acumulado_usd != 0:
			self.acumulado_pciento_usd = self.acumulado_usd / self.rm_report_intercompanias_id.total_general_usd
		else:
	...
	def get_promedio_pciento_usd(self):
		if self.acumulado_usd != 0:
			self.promedio_pciento_usd = self.promedio_usd / self.rm_report_intercompanias_id.total_promedio_general_usd
		else:
			self.promedio_pciento_usd = 0
	promedio_pciento_usd = fields.Float('%  PROM', readonly=True, compute="get_promedio_pciento_usd")

class rm_report_intercompanias(models.Model):
	_inherit= 'rm.report.intercompanias'
	...
	def export_excel_usd(self):
		...
		workbook = Workbook(direccion +'Reporte_Intercompañias_USD.xlsx')
		worksheet = workbook.add_worksheet(u"Intercompañias")
		...
		elements = self.env['rm.report.intercompanias.line'].search([('rm_report_intercompanias_id','=',self.id)]).sorted(key=lambda r: dig_5(r.tipo.order)+dig_5(r.grupo.order))
		flag = True
		n_grupo = None
		n_tipo = None
		ultimo_elem = None
		...
		f = open(direccion + 'Reporte_Intercompañias_USD.xlsx', 'rb')
		
		vals = {
			'output_name': 'Reporte_Intercompañias_USD.xlsx',
