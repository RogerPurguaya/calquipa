...
class rm_report_capacitacion(models.Model):
	...
	def export_excel_usd(self):
		...
		workbook = Workbook(direccion +'Reporte_Capacitación_USD.xlsx')
		worksheet = workbook.add_worksheet(u"Capacitación")
		...
		elements = self.env['rm.report.capacitacion.line'].search([('rm_report_capacitacion_id','=',self.id)]).sorted(key=lambda r: dig_5(r.tipo.order)+dig_5(r.grupo.order))
		flag = True
		n_grupo = None
		n_tipo = None
		ultimo_elem = None
		...
		f = open(direccion + 'Reporte_Capacitación_USD.xlsx', 'rb')
		
		vals = {
			'output_name': 'Reporte_Capacitación_USD.xlsx',
